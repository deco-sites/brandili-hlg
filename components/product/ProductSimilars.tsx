          ...
          {unique?.map((item: any, index: number) => {
            return (
              <label
                ...
                hx-get={useSection({ href: `/${item?.linkText}/p` })}
              >
                <Image
                  src={item?.items[0]?.images[0]?.imageUrl}
                  class="w-12 h-auto rounded-sm border border-solid border-gray-11 block [.htmx-request_&]:hidden"
                  width={48}
                  height={65}
                />
                <div class="w-[52px] h-10 justify-center items-center hidden [.htmx-request_&]:flex">
                  <span
                    class={clx(ringClass, "loading loading-xs loading-spinner")}
                  />
                </div>
              </label>
            );
