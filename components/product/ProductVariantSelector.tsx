...
export const ringClass = clx("h-6 w-6", "rounded-full text-center", "ring-1 ring-offset-4", "ring-base-300 peer-checked:ring-base-content");
      ...
      {Object.keys(possibilities).map((name) => (<li class="flex flex-col gap-2">
          ...
          <ul class="flex flex-row gap-1 flex-wrap">
            {Object.entries(possibilities[name]).map(([value, link]) => {
                ...
                return (<li>
                  <label class="cursor-pointer" hx-get={useSection({ href: relativeLink })}>
                    {/* Checkbox for radio button on the frontend */}
                    <input class="hidden peer" type="radio" name={name} checked={relativeLink === relativeUrl}/>
                    <div class="block [.htmx-request_&]:hidden">
                      <Avatar content={value} variant={relativeLink === relativeUrl &&
