...
function Newsletter({ content = {} }: Props, { device }: Props) {
    ...
    const handleSubmit: JSX.GenericEventHandler<HTMLFormElement> = async (e) => {
        ...
        try {
            ...
            await fetch("https://brandili.app.n8n.cloud/webhook/b44993d7-119e-4520-86eb-801f9800af50", {
              method: "POST",
              body: JSON.stringify({
                "nome": name,
                "email": email,
    ...
    return (<div class="flex flex-col gap-y-3.5 bg-gray-9 w-full py-8 px-4">
      <div class="w-full max-w-[1440px] m-auto">
        ...
        <div class="flex flex-col gap-4 mt-3">
          <form id="form-newsletter" class="flex flex-col gap-4 form-control relative" onSubmit={handleSubmit}>
            <div class="grid justify-between gap-4 grid-cols-[37%_37%_23%] full-phone:grid-cols-1">
              ...
              {content?.form?.helpText && device === "mobile" && (<div class="checkbox-newsletter flex flex-row sm:hidden items-center">
                  ...
                  <label class="text-sm" dangerouslySetInnerHTML={{
                __html: content?.form?.helpText,
            }}/>
                </div>)}
              <button type="submit" class={`disabled:loading h-10 min-h-10 font-normal text-xs rounded w-full  ${content?.form?.buttonColor ?? "btn"}`} disabled={loading}>
                {content?.form?.buttonText || "Cadastrar"}
              </button>
            </div>
            {content?.form?.helpText && (<div class="checkbox-newsletter flex flex-row items-center">
                <input class="aceite rounded" type="checkbox" id="aceite" name="terms" required/>
                <span class="check-aceite"></span>
                <label class="text-sm" dangerouslySetInnerHTML={{ __html: content?.form?.helpText }}/>
              </div>)}
            <div class={`${loading.value ? "flex" : "hidden"} message-success items-center justify-center bg-gray-9 font-semibold text-green-0 text-2xl absolute left-0 -top-1.5 right-0 h-full w-full`}>
